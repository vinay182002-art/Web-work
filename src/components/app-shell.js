import { demoCurriculum, getChapter, getDefaultChapter, getSubject } from '../data/curriculum.js?v=23';
...
import { getGlossary, searchContent } from '../services/content-engine.js?v=21';
import { analyseAttempts, getQuestions, getQuestionTree, selectPracticeQuestions } from '../services/question-engine.js?v=21';
...
import { getForensicChemistryAudit, getFingerprintScienceAudit, getForensicStatisticsAudit, getSubjectCompletionPipeline } from '../services/subject-audit.js?v=21';
import { getTopicProgress, renderTopicDeepDive, renderTopicProgress, renderTopicSummary, renderTopicTest, renderTopicVisual } from './topic-learning.js?v=24';
import { renderSemesterDashboard } from './semester-dashboard.js?v=1';
import { renderSubjectDashboard } from './subject-dashboard.js?v=1';
import { renderRevisionView } from './revision-view.js?v=1';
import { renderPapersView } from './papers-view.js?v=1';
import { renderBookmarksView } from './bookmarks-view.js?v=1';
import { buildMentorContext } from '../services/ai.js';
...
function renderRoute(route, dashboard) {
  ...
  if (segment === 'revision') return renderRevisionView(dashboard.state);
  if (segment === 'bookmarks') return renderBookmarksView(dashboard.state);
  if (segment === 'papers') return renderPapersView();
  ...
  if (segment === 'learn' && year && semester && !subjectId) return renderSemesterDashboard(dashboard);
  ...
  function renderQuestionCard(question, index, state) {
    ...
    const bookmarked = (state.bookmarks || []).includes(`question:${question.id}`);
    let sourceContext = '';
    const scopeChapter = getQuestionTree().flatMap((subject) => subject.units.flatMap((unit) => unit.chapters)).find((chapter) => chapter.chapterId === question.chapterId);
    if (scopeChapter) {
      const subject = getQuestionTree().find((item) => item.units.some((unit) => unit.chapters.some((chapter) => chapter.chapterId === question.chapterId)));
      sourceContext = `<span>${subject?.title || 'Semester 3'}</span><span>${scopeChapter.title}</span>`;
    }
    return `<article class="question-card" data-question-card="${question.id}"><div class="question-card-header"><span class="card-label">Q${index + 1} · ${question.questionType} · ${question.marks} mark${question.marks === 1 ? '' : 's'}</span><span class="question-badge">${question.difficulty}</span><button class="bookmark-button ${bookmarked ? 'is-bookmarked' : ''}" data-bookmark-question="${question.id}" aria-label="Bookmark question">${bookmarked ? '★' : '☆'}</button></div><h3>${question.question}</h3><div class="question-options">${question.options.map((option, optionIndex) => `<button data-question-answer="${question.id}:${optionIndex}" class="${attempt ? (optionIndex === question.correctAnswer ? 'is-correct' : attempt.selectedAnswer === optionIndex ? 'is-incorrect' : '') : ''}">${option}</button>`).join('')}</div><div class="question-meta"><span>${question.bloomLevel}</span><span>${question.relatedTopic}</span><span>${question.sourceType.replaceAll('_', ' ')}</span>${sourceContext}</div><div class="question-feedback" id="question-feedback-${question.id}">${attempt ? `<strong>${attempt.correct ? 'Correct' : 'Review this concept'}</strong><p>${question.explanation}</p><p><strong>What to revise:</strong> ${question.recommendedLesson} · ${question.commonMisconception}</p>` : 'Choose an answer to see the explanation and next revision step.'}</div></article>`;
  }
...
function renderSemesterEntryCard() {
  return `<section class="hero-panel sem-entry-panel" data-searchable>
    <div class="hero-header"><span class="soft-label">Semester 3 · study platform</span><span class="time-label">Year 2 · B.Sc. Forensic Science</span></div>
    <div class="hero-grid">
      <div class="hero-copy-block">
        <p class="mini-label">Your semester at a glance</p>
        <h2>Semester 3 dashboard.</h2>
        <p class="hero-sub">Overall progress, subjects, topics, quiz accuracy, streak, weak topics, and smart revision — one place.</p>
        <button class="resume-btn" data-route="${routes.semester(2, 3)}">Open Semester 3 <span>↗</span></button>
      </div>
      <div class="sem-shortcut-stack">
        <a class="text-link" data-route="${routes.revision}">Smart Revision ↗</a>
        <a class="text-link" data-route="${routes.papers}">Previous Year Papers ↗</a>
        <a class="text-link" data-route="${routes.bookmarks}">My Bookmarks ↗</a>
      </div>
    </div>
  </section>`;
}
...
function renderChapter(chapter, state, subjectId = 'forensic-biology', year = 2, semester = 3) {
  ...
  const tab = new URLSearchParams(window.location.hash.split('?')[1] || '').get('tab') || state.topicModes?.[chapter.id]?.lastMode || 'deep-dive';
  const revision = state.revisionItems.find((item) => item.chapterId === chapter.id);
  const topicProgress = getTopicProgress(chapter, state);
  return `<section class="page-intro chapter-page"><a class="back-link" data-route="${routes.subject(year, semester, subjectId)}">← ${subjectId === 'forensic-biology' ? 'Forensic Biology' : subjectId === 'forensic-chemistry' ? 'Forensic Chemistry' : subjectId.replaceAll('-', ' ')}</a><div class="chapter-heading"><div><p class="eyebrow">${chapter.metadata.contentLevel === 'foundation' ? 'Foundation' : chapter.metadata.contentLevel === 'advanced_undergraduate_content' ? 'Advanced undergraduate' : 'Supplementary / Standard Curriculum'} · ${chapter.estimatedMinutes} minutes</p><h1>${chapter.title}</h1><p>${chapter.description}</p></div><button class="bookmark-button ${state.bookmarks.includes(chapter.id) ? 'is-bookmarked' : ''}" data-bookmark="${chapter.id}">${state.bookmarks.includes(chapter.id) ? '★ Bookmarked' : '☆ Bookmark'}</button></div><div class="chapter-meta"><span>Difficulty: ${chapter.difficulty}</span><span>Relevance: ${chapter.universityRelevance}</span><span>Mastery: ${progress}%</span></div>${renderTopicProgress(chapter, state)}
    <nav class="chapter-tabs" aria-label="Chapter sections">${[['deep-dive','Deep Dive'],['summary','Quick Summary'],['visual','Visual Revision'],['quiz','Test Yourself'],['practical','Practical'],['case','Case'],['flashcards','Flashcards'],['notes','Notes'],['ai','AI Tutor']].map(([key, label]) => `<button data-tab="${key}" class="${tab === key ? 'is-active' : ''}">${label}</button>`).join('')}</nav>
    <div class="chapter-layout"><div class="chapter-main">${renderChapterTab(chapter, state, tab, subjectId, year, semester)}</div><aside class="chapter-aside"><p class="card-label">Topic progress · ${masteryLabel(progress)}</p><strong class="aside-progress">${topicProgress.overall}%</strong><p>${attempt ? `Latest test: ${attempt.score}/${attempt.total}` : 'Complete a learning mode and test yourself to establish progress.'}</p>${revision ? `<p class="revision-note">Next review: ${new Date(revision.nextReviewAt).toLocaleDateString('en-IN')}<br />${revision.reason}</p>` : ''}<button class="text-link" data-route="${routes.quiz(year, semester, subjectId, chapter.id)}">Open dedicated quiz ↗</button><button class="text-link" data-reset-topic="${chapter.id}">Reset topic progress</button></aside></div>
  </section>`;
}

function renderChapterTab(chapter, state, tab, subjectId = 'forensic-biology', year = 2, semester = 3) {
  if (tab === 'summary') return renderTopicSummary(chapter);
  if (tab === 'visual') return renderTopicVisual(chapter);
  ...
  if (tab === 'quiz') return renderTopicTest(chapter, state);
  ...
  if (tab === 'ai') {
    const mentorContext = buildMentorContext({
      profile: { programme: 'B.Sc. Forensic Science', year, semester },
      chapter,
      state,
      mastery: state.progressByChapter[chapter.id] || 0,
      subject: getSubject(subjectId),
      learningMode: tab,
      currentSection: tab,
      studentQuestion: ''
    });
    return `<div class="reading-stack"><p class="eyebrow">Contextual AI boundary</p><h2>Ask about this chapter</h2><p class="ai-context">Context supplied automatically: Year ${year} · Semester ${semester} · ${subjectId.replaceAll('-', ' ')} · ${chapter.title} · ${tab} mode · mastery ${state.progressByChapter[chapter.id] || 0}%.</p><label class="ai-question-label" for="ai-question">Your question</label><textarea class="note-input" id="ai-question" rows="2" placeholder="e.g. Why is ethanol used in the DNA precipitation stage here?"></textarea><div class="quick-actions">${['Explain simply','Explain deeply','Give an analogy','Quiz me','Give exam answer','Connect to a case'].map((action) => `<button data-ai-action="${action}">${action}</button>`).join('')}</div><div class="content-card"><p id="ai-response">AI answers will be connected to a server-side, grounded model after the knowledge base and provider are configured.</p></div><details class="ai-context-details"><summary>Context this tutor receives</summary><pre class="ai-context-json">${escapeHtml(JSON.stringify(mentorContext, null, 2))}</pre></details></div>`;
  }
  return renderTopicDeepDive(chapter, state);
}
...
function renderDrawer() {
  const links = [
    ...
    ['Semester 3', routes.semester(2, 3)],
    ['Learn', routes.learn],
    ['Subjects', '#/subjects'],
    ...
    ['Smart revision', routes.revision],
    ['My bookmarks', routes.bookmarks],
    ['Previous year papers', routes.papers],
